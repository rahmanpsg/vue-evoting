...
export default {
  ...
  methods: {
    ...
    setFaceRecognition() {
      const item = this.$store.state.pemilihModule.items[this.index];

      Object.assign(item, { ...item, face_recognition: true });

      this.$store.commit("pemilihModule/setItem", this.index, item);
